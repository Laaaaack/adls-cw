...
//-----------------------------------------------------------------------
torch::Tensor reduce_sum_inner_dim(const torch::Tensor input) {
  ...
  auto output =
      torch::zeros(output_sizes, torch::dtype(input.dtype())
                                     .device(input.device())
                                     .layout(input.layout())
  ...
  if (input.dtype() == torch::ScalarType::Half) {
    ...
    reduce_sum_kernel<float><<<blocks, threads>>>(input.data_ptr<float>(),
                                                  total_size, inner_dim_size,
...
torch::Tensor reduce_max_inner_dim(const torch::Tensor input) {
  at::Device device = input.device();
  const at::cuda::CUDAGuard device_guard{device};
  const long inner_dim_size = get_inner_dim_size(input);
  const long total_size = get_total_size(input);
  ...
  if (input.dtype() == torch::ScalarType::Half) {
    ...
    reduce_max_kernel<float><<<blocks, threads>>>(input.data_ptr<float>(),
                                                  total_size, inner_dim_size,
                                                  output.data_ptr<float>());
  }
