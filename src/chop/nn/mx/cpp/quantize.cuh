...
shift_right_round_mantissa(int &mantissa, // 23-bit float32 trailing mantissa
...
shift_left_mantissa(int &mantissa,         // From shift_right_round_mantissa
                    const bool is_subnorm, // is the input a subnorm?
                    const int mbits, const int exp_diff) {
  int fp32_sig_bits = is_subnorm ? 23 : 24;
...
quantize_elemwise(float input,
                  int bits,     // bits = mantissa bits + sign bit
                  int exp_bits, // exp_bits == 0 indicates integer dtype
                  float max_norm, const RoundingMode rounding_mode = rd_away,
                  bool saturate_normals = false, bool allow_denorm = true) {
  u_float_int input_;
  input_.f = input;
  ...
  shift_right_round_mantissa(tmant, biased_exp == 0, mbits, exp_diff,
                             rounding_mode, !is_int);
...
// Half-precision
//-----------------------------------------------------------------------
__host__ __device__ __forceinline__ __half
quantize_elemwise(__half input, int bits, int exp_bits, float max_norm,
                  const RoundingMode rounding_mode = rd_away,
                  bool saturate_normals = false, bool allow_denorm = true) {
  float in_fp32 = __half2float(input);
  float out_fp32 =
      quantize_elemwise(in_fp32, bits, exp_bits, max_norm, rounding_mode,
