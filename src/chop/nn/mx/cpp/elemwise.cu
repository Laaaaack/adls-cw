...
//-----------------------------------------------------------------------
torch::Tensor quantize_elemwise_cuda(const torch::Tensor input, const int bits,
                                     const int exp_bits, const float max_norm,
  ...
  if (input.dtype() == torch::ScalarType::Half) {
    ...
    quantize_elemwise_cuda_kernel<<<blocks, threads>>>(
        (__half *)input.data_ptr<at::Half>(), bits, exp_bits, max_norm,
        total_size, rounding_mode, saturate_normals, allow_denorm,
    ...
    quantize_elemwise_cuda_kernel<<<blocks, threads>>>(
        input.data_ptr<float>(), bits, exp_bits, max_norm, total_size,
