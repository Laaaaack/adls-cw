...
//-----------------------------------------------------------------------
template <typename T> class SumReduceHelper {
public:
  static const T start_val;

  // Reduces two values
  static T reduce(const T a, const T b) { return a + b; }

  // Reduce across the threads of a single warp, result in thread 0
  __forceinline__ __device__ static T reduce_warp(T val,
                                                  const int size = WARP_SIZE) {
  ...
  __forceinline__ __device__ static void atomic_accum(T *addr, T val) {
    atomicAdd(addr, val);
  }
};

template <typename T> class MaxReduceHelper {
public:
  static const T start_val;

  // Reduces two values
  static T reduce(const T a, const T b) { return (a > b) ? a : b; }
...
__forceinline__ __device__ T reduce_block(T val, const long total_size,
...
template <typename T>
__forceinline__ __device__ T reduce_sum_block(T val, const long total_size,
                                              const long inner_dim_size) {
  return reduce_block<SumReduceHelper, T>(val, total_size, inner_dim_size);
}

template <typename T>
__forceinline__ __device__ T reduce_max_block(T val, const long total_size,
                                              const long inner_dim_size) {
...
reduce_kernel(const T *__restrict__ in, const long total_size,
              const long inner_dim_size, T *__restrict__ out) {
...
template <typename T>
__global__ void
reduce_sum_kernel(const T *__restrict__ in, const long total_size,
                  const long inner_dim_size, T *__restrict__ out) {
...
template <typename T>
__global__ void
reduce_max_kernel(const T *__restrict__ in, const long total_size,
                  const long inner_dim_size, T *__restrict__ out) {
