...
//-----------------------------------------------------------------------
inline long get_blocks(long size, int max_threads = MAX_THREADS) {
  long blocks;
  if (size <= max_threads) {
    blocks = 1;
  } else {
...
inline int get_threads(long size, int max_threads = MAX_THREADS) {
...
get_biased_exponent(const u_float_int input) {
...
__host__ __device__ __forceinline__ int
get_unbiased_exponent(const float input) {
  u_float_int u;
  u.f = input;
...
__host__ __device__ __forceinline__ int
get_biased_exponent(const __half input) {
  u_float_int u;
...
__host__ __device__ __forceinline__ int
get_trailing_mantissa(const u_float_int input) {
