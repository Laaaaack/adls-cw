...
// quantize_mx_cuda
//-----------------------------------------------------------------------
torch::Tensor quantize_mx_cuda(const torch::Tensor input, const int scale_bits,
                               const int elem_ebits, const int elem_mbits,
                               const float elem_max_norm,
                               const torch::Tensor max_values, const int axis,
  ...
  if (input.dtype() == torch::ScalarType::Half) {
    ...
    quantize_mx_cuda_kernel<<<blocks, threads>>>(
        input.data_ptr<float>(), scale_bits, elem_ebits, elem_mbits,
        elem_max_norm, max_values.data_ptr<float>(), total_size, axis_size,
        post_axis_size, flush_fp32_subnorms, rounding_mode,
...
//-----------------------------------------------------------------------
torch::Tensor
quantize_mx_by_tile_cuda(const torch::Tensor input, const int scale_bits,
                         const int elem_ebits, const int elem_mbits,
                         const float elem_max_norm, const int tile_size,
                         const int axis, const bool flush_fp32_subnorms = false,
                         const RoundingMode rounding_mode = rd_away) {
  at::Device device = input.device();
  const at::cuda::CUDAGuard device_guard{device};
  auto output = torch::empty_like(input);
  ...
  if (axis == ndim - 1 && axis_size % tsize == 0 && tsize <= WARP_SIZE &&
      is_power_of_two(tsize)) {
    ...
    if (input.dtype() == torch::ScalarType::Half) {
      ...
      quantize_mx_innermost_cuda_kernel<<<blocks, threads>>>(
          input.data_ptr<float>(), scale_bits, elem_ebits, elem_mbits,
          elem_max_norm, total_size, tsize, flush_fp32_subnorms, rounding_mode,
          output.data_ptr<float>());
    }
  }
  // Otherwise call quantize_mx_by_tile
    ...
    if (input.dtype() == torch::ScalarType::Half) {
      ...
      quantize_mx_by_tile_cuda_kernel<<<blocks, threads>>>(
          input.data_ptr<float>(), scale_bits, elem_ebits, elem_mbits,
          elem_max_norm, total_tiles, tsize, num_tiles, axis_size,
          post_axis_size, flush_fp32_subnorms, rounding_mode,
          output.data_ptr<float>());
    }
  }
