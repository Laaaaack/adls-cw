...
//-----------------------------------------------------------------------
torch::Tensor
quantize_mx_func_cpp(torch::Tensor A, // Input tensor
                     int scale_bits, int elem_ebits, int elem_mbits,
                     float elem_max_norm,
                     torch::Tensor max_values, // Max values in each block
                     int axis, // Axis along which exponents are shared
                     const bool flush_fp32_subnorms, const int rmode = 0) {
  const int ndim = A.dim();
  AT_ASSERTM(1 <= ndim && ndim <= 5, " number of dims outside of range [1,5]");
  AT_ASSERTM(axis >= 0 && axis < ndim,
             " shared axis < 0 or shared axis >= ndim");
  ...
  for (int i = 0; i < pre_axis_size; i++) {
    ...
    for (int j = 0; j < post_axis_size; j++) {
      ...
      for (int k = 0; k < axis_size; k++) {
        ...
        float scaled_out =
            quantize_elemwise(scaled_in, elem_mbits, elem_ebits, elem_max_norm,
...
torch::Tensor quantize_elemwise_func_cpp(
    ...
    float max_norm, const int rmode = 0, const bool saturate_normals = false,
  ...
  for (long i = 0; i < total_size; i++) {
    o_data[i] =
        quantize_elemwise(i_data[i], bits, exp_bits, max_norm, rounding_mode,
...
torch::Tensor quantize_mx_func_cuda(
    ...
    int scale_bits, int elem_ebits, int elem_mbits, float elem_max_norm,
    torch::Tensor max_values, // Max values along axis of interest
    int axis,                 // Axis along which exponents are shared
    const bool flush_fp32_subnorms = false, const int rmode = 0) {
  CHECK_INPUT(A);
  AT_ASSERTM(axis < A.dim(),
             " quantize_mx_func_cuda axis exceeds input dimensions");
  CHECK_INPUT(max_values);

  // Explicitly cast int to enum
  RoundingMode rounding_mode = static_cast<RoundingMode>(rmode);

  return quantize_mx_cuda(A, scale_bits, elem_ebits, elem_mbits, elem_max_norm,
                          max_values, axis, flush_fp32_subnorms, rounding_mode);
}

torch::Tensor
quantize_mx_by_tile_func_cuda(torch::Tensor A, // Input tensor
                              int scale_bits, int elem_ebits, int elem_mbits,
                              float elem_max_norm, int tile_size, int axis,
                              const bool flush_fp32_subnorms = false,
                              const int rmode = 0) {
  CHECK_INPUT(A);
  AT_ASSERTM(axis < A.dim(),
             " quantize_mx_by_tile axis exceeds input dimensions");
...
torch::Tensor quantize_elemwise_func_cuda(
  ...
  CHECK_INPUT(A);
  AT_ASSERTM(bits <= 24,
             " quantize_elemwise with bits > 24 leads to negative shifts");

  // Explicitly cast int to enum
  RoundingMode rounding_mode = static_cast<RoundingMode>(rmode);

  return quantize_elemwise_cuda(A, bits, exp_bits, max_norm, rounding_mode,
                                saturate_normals, allow_denorm);
}

torch::Tensor reduce_sum_inner_dim_cuda(torch::Tensor A) {
  CHECK_INPUT(A);
  return reduce_sum_inner_dim(A);
}

torch::Tensor reduce_max_inner_dim_cuda(torch::Tensor A) {
  CHECK_INPUT(A);
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("quantize_mx_func_cpp", &quantize_mx_func_cpp,
        "MX quantization function in C++");
  m.def("quantize_elemwise_func_cpp", &quantize_elemwise_func_cpp,
        "Element-wise quantization function in C++");
  m.def("quantize_mx_func_cuda", &quantize_mx_func_cuda,
        "MX quantization function with CUDA");
  m.def("quantize_mx_by_tile_func_cuda", &quantize_mx_by_tile_func_cuda,
        "MX quantization function with CUDA");
  m.def("quantize_elemwise_func_cuda", &quantize_elemwise_func_cuda,
        "Element-wise quantization function with CUDA");
  m.def("reduce_sum_inner_dim", &reduce_sum_inner_dim_cuda,
        "Sum reduction over innermost dim");
  m.def("reduce_max_inner_dim", &reduce_max_inner_dim_cuda,
        "Max reduction over innermost dim");
}
