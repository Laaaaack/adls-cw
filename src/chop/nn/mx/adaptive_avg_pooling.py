...
class AdaptiveAvgPool2d(nn.Module):
    ...
    def forward(self, inputs):
        ...
        return AdaptiveAvgPool2dFunction.apply(
            inputs, self.output_size, self.mx_specs, self.name
        )
...
class AdaptiveAvgPool2dFunction(torch.autograd.Function):
    ...
    def forward(ctx, input, output_size, mx_specs=None, name=None):
        ...
        if isinstance(output_size, tuple) and len(output_size) == 2:  # image H x W
            osizeH = (
                output_size[0] if output_size[0] else isizeH
            )  # need to support the case of None in output size
            osizeW = (
                output_size[1] if output_size[1] else isizeW
            )  # need to support the case of None in output size
            ...
            raise ValueError(
                "expected 1D or 2D output_size (got {}D output_size)".format(
                    len(output_size)
                )
            )
        ...
        for oh in range(osizeH):
            ...
            for ow in range(osizeW):
                ...
                output[:, :, oh, ow] = vec_reduce_mean(
                    input_slice, [2, 3], keepdim=False, mx_specs=mx_specs
                )
    ...
    def backward(ctx, grad_output):
        osizeH, osizeW, sizeB, sizeD, isizeH, isizeW, device = (
            ctx.osizeH,
            ctx.osizeW,
            ctx.sizeB,
            ctx.sizeD,
        ...
        for oh in range(osizeH):
            ...
            for ow in range(osizeW):
                ...
                expanded_grad_delta = grad_delta.view(
                    *grad_delta.shape, *(1,) * (len(target_shape) - grad_delta.ndim)
                ).expand(target_shape)
