...
class BatchNormFunction(torch.autograd.Function):
    ...
    def forward(
        ...
        if is_training and running_mean is not None:
            t1 = vec_mul((1 - momentum), running_mean, mx_specs=mx_specs)
            t2 = vec_mul(momentum, x_mean.view(H), mx_specs=mx_specs)
            t3 = vec_add(t1, t2, mx_specs=mx_specs)
            running_mean.copy_(t3)
        if is_training and running_var is not None:
            t1 = vec_mul((1 - momentum), running_var, mx_specs=mx_specs)
            t2 = vec_mul(momentum, x_var.view(H), mx_specs=mx_specs)
            t3 = vec_add(t1, t2, mx_specs=mx_specs)
...
def batch_norm(
    ...
    if mx_specs is None:
        return f_batch_norm(
            x, running_mean, running_var, weight, bias, is_training, momentum, eps
        )

    mx_specs = apply_mx_specs(mx_specs)
    return BatchNormFunction.apply(
        x, running_mean, running_var, weight, bias, is_training, momentum, eps, mx_specs
    )


class _BatchNorm(torch.nn.modules.batchnorm._BatchNorm):
    def __init__(self, num_features, mx_specs=None, name=None, **kwargs):

        # Early pytorch did not have device/dtype args
        try:
            super(_BatchNorm, self).__init__(num_features, **kwargs)
        except TypeError:
