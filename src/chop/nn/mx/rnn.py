...
class LSTM(torch.nn.LSTM):
    ...
    def _cell(self, xw, hc, layer, reverse=False):
        ...
        # gates (1, N, 4*Hc)
        linear_out = linear(
            h, W_h, bias=b_h, mx_specs=self.mx_specs, name=self.name + ".HiddenLinear"
        )
    ...
    def _hidden_proj_packed_r(self, hx_r, inputs_w, batch_sizes, layer):
        ...
        for i in range(len(batch_sizes) - 1, -1, -1):
            ...
            if inc > 0:
                hidden_r = self._hx_cat(
                    [hidden_r, self._hx_slice(hx_r, last_batch_size, batch_size)], dim=1
                )
    ...
    def forward(self, input, hx=None):
        ...
        for l in range(self.num_layers):
            ...
            # Hidden projections
            if isinstance(orig_input, PackedSequence):
                hx, step_outputs = self._hidden_proj_packed(
                    hx, inputs_w, batch_sizes, l
                )

                if self.bidirectional:
                    hx_r, step_outputs_r = self._hidden_proj_packed_r(
                        hx_r, inputs_w_r, batch_sizes, l
                    )
            else:
                step_outputs = []
                step_outputs_r = []
                ...
                for i in range(L):
                    ...
                    if self.bidirectional:
                        ...
                        hx_r = self._cell(inputs_w_r[-1 - i], hx_r, l, reverse=True)
        ...
        hidden = (torch.cat(final_hs, dim=0), torch.cat(final_cs, dim=0))

        if isinstance(orig_input, PackedSequence):
            output_packed = PackedSequence(
                step_outputs, batch_sizes, sorted_indices, unsorted_indices
            )
