...
from spikingjelly.activation_based.auto_cuda.generator import (
    analyse_graph,
    gen_forward_codes,
    gen_backward_codes,
)
...
if __name__ == "__main__":
    ...
    input_nodes, inter_nodes, output_nodes, cmds = analyse_graph(
        lif_charge, requires_grad=(True, True, False, False)
    )

    forward_codes, forward_kernel_name, cuda_cmds = gen_forward_codes(
        input_nodes, inter_nodes, output_nodes, cmds, hard_reset=True
    )

    backward_codes, backward_kernel_name, input_bp_vars = gen_backward_codes(
        cuda_cmds,
        input_nodes,
        output_nodes,
        cmds,
