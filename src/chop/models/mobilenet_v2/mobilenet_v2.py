...
def get_mobilenet_v2(info: Dict, pretrained: bool = False, **kwargs: Any):
    ...
    if pretrained:
        ...
        if num_classes != 1000:
            ...
            logger.warning(
                f"The num_classes={num_classes} != 1000. The classifier head randomly initialized. "
            )
        model.load_state_dict(pretrained_weight, strict=False)
        logger.info("Pretrained weights loaded into MobileNetV2.")
    else:
        logger.info("Randomly initialized MobileNetV2.")
