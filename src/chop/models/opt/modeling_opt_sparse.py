...
from .configuration_opt_sparse import OPTSparseConfig
...
_CONFIG_FOR_DOC = "OPTSparseConfig"
...
class OPTAttention(nn.Module):
    ...
    def __init__(
        ...
        self.k_proj = LinearSparse(
            in_features=embed_dim,
            out_features=embed_dim,
            bias=bias,
            config=sparse_config["k_proj"],
        )
        self.v_proj = LinearSparse(
            in_features=embed_dim,
            out_features=embed_dim,
            bias=bias,
            config=sparse_config["v_proj"],
        )
        self.q_proj = LinearSparse(
            in_features=embed_dim,
            out_features=embed_dim,
            bias=bias,
            config=sparse_config["q_proj"],
        )
        self.o_proj = LinearSparse(
            in_features=embed_dim,
            out_features=embed_dim,
            bias=bias,
            config=sparse_config["o_proj"],
        )
...
class OPTDecoderLayer(nn.Module):
    def __init__(self, config: OPTSparseConfig):
        super().__init__()
