...
class OPTQuantizedDecoderLayer(nn.Module):
    def __init__(self, config: OPTQuantizedConfig, layer_id: int):
        ...
        # by default, we do an integer 16-bit quantisation
        if config.quant_config is None:
            self_attn_quant_config = {
                "k_proj": default_quant_config,
                "q_proj": default_quant_config,
                "v_proj": default_quant_config,
                "out_proj": default_quant_config,
            }
            fc1_quant_config = default_quant_config
            fc2_quant_config = default_quant_config
        else:
            self_attn_quant_config = config.quant_config[f"model_layer_{layer_id}"][
                "self_attn"
            ]
