...
if find_spec("pytorch_quantization") is None or find_spec("tensorrt") is None:
    ...
    def prepare_save_path(*args, **kwargs):
        raise ImportError(
            "The `pytorch_quantization` and `tensorrt` packages are required for this feature."
        )

    def check_for_value_in_dict(*args, **kwargs):
        raise ImportError(
            "The `pytorch_quantization` and `tensorrt` packages are required for this feature."
        )
    ...
    from chop.passes.utils import get_mase_op, get_node_actual_target, get_parent_name
