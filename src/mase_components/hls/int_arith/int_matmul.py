...
def int_matmul_gen(
    ...
    """
    This script generates a fixed-point matmul in HLS
    """
    ...
    y_int_width = x_int_width + w_int_width
    ...
    type_w = get_fixed_ty(w_row, w_col, w_width, w_frac_width)
    if type_w not in writer.types:
        writer.type_buff += new_fixed_ty(w_row, w_col, w_width, w_frac_width)
...
"""
    ...
    linear_buff = """
// Matmul:
void int_matmul_{}(hls::stream<{}> &data_in, hls::stream<{}> &data_out) {{
