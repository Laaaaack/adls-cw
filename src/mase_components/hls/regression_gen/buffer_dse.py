...
from hls.elastic import buffer_gen
from hls import HLSWriter


def buffer_dse(mode=None, top=None, threads=16):
    ...
    print("Exploring elastic buffer. Design points = {}".format(size))
        ...
        for x_col in x_cols:
            for x_width in x_widths:
                for x_frac_width in x_frac_widths:
                    ...
                    file_name = f"x{i}_buffer_{x_row}_{x_col}_{x_width}_{x_frac_width}"
                    ...
                    if mode in ["codegen", "all"]:
                        ...
                        top_name = f"buffer_{writer.op_id-1}"
