...
def bfp_add_dse(mode=None, top=None, threads=16):
    ...
    # x_exp_widths = [1, 2, 3, 4, 5, 6, 7, 8]
    x_exp_widths = [8]
    x_man_widths = [1, 2, 3, 4, 5, 6, 7, 8]
    # w_exp_widths = [1, 2, 3, 4, 5, 6, 7, 8]
    w_exp_widths = [8]
                ...
                for x_man_width in x_man_widths:
                    for w_exp_width in w_exp_widths:
                        for w_man_width in w_man_widths:
                            ...
                            file_name = f"x{i}_bfp_add_{x_row}_{x_col}_{x_exp_width}_{x_man_width}_{w_exp_width}_{w_man_width}"
                            ...
                            if mode in ["codegen", "all"]:
                                ...
                                top_name = f"bfp_add_{writer.op_id-1}"
    ...
    if mode in ["codegen", "all"]:
        ...
        bash_gen(commands, top, "bfp_add")

    if mode in ["report", "all"]:
        # Export regression model data points to csv
        csv_gen(data_points, top, "bfp_add_hw")

    if mode in ["count_loc", "all"]:
        # Export regression model data points to csv
        csv_gen(loc_points, top, "bfp_add_loc")
