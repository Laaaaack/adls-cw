...
from hls.int_arith import int_silu_gen
from hls import HLSWriter


def int_silu_dse(mode=None, top=None, threads=16):
    ...
    print("Exploring silu. Design points = {}".format(size))
        ...
        for x_col in x_cols:
            for x_width in x_widths:
                for x_frac_width in x_frac_widths:
                    ...
                    file_name = (
                        f"x{i}_int_silu_{x_row}_{x_col}_{x_width}_{x_frac_width}"
                    )
                    ...
                    if mode in ["codegen", "all"]:
                        ...
                        top_name = f"int_silu_{writer.op_id-1}"
    ...
    if mode in ["codegen", "all"]:
        ...
        bash_gen(commands, top, "int_silu")

    if mode in ["report", "all"]:
        # Export regression model data points to csv
        csv_gen(data_points, top, "int_silu_hw")

    if mode in ["count_loc", "all"]:
        # Export regression model data points to csv
        csv_gen(loc_points, top, "int_silu_loc")
