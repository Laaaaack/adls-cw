from .bfp_multiplier import bfp_multiplier_gen
from .utils import clog2, get_bfp_ty, new_bfp_ty


def bfp_block_multiplier_gen(
    writer,
    x_exp_width=16,
    x_man_width=8,
    x_row=3,
    ...
    """
    This script generates a block-level bfp mult in HLS
    """
    ...
    multiplier_id = writer.op_id
    writer = bfp_multiplier_gen(
        writer,
...
bfp_multiplier_{multiplier_id}(sign_{i}_{j}_0, exp_{i}_{j}_0, man_{i}_{j}_0, sign_{i}_{j}_1, exp_{i}_{j}_1, man_{i}_{j}_1, &sign_{i}_{j}_2, &exp_{i}_{j}_2, &man_{i}_{j}_2);
...
"""
    ...
    buff = f"""
// BFP Add Block:
void bfp_block_multiplier_{op_id}({type_in0} d0, 
{type_in1} d1, 
{type_out} *data) {{
#pragma HLS INLINE
{body}
