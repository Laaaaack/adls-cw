...
class VerificationCase:
    def __init__(self, samples=10):
        ...
        self.num = 9
        self.data_out_width = math.ceil(math.log2(self.num))
    ...
    def sw_compute(self):
        ...
        for i in range(self.samples):
            ref.append(
                sum([utils.binary_decode(element) for element in self.inputs.data[i]])
            )
        ref.reverse()
        return ref
...
def runner():
    ...
    runner.build(
        ...
        hdl_toplevel="binary_activation_binary_adder_tree",
        build_args=extra_args,
    )

    runner.test(
        hdl_toplevel="binary_activation_binary_adder_tree",
        test_module="binary_activation_binary_adder_tree_tb",
    )
